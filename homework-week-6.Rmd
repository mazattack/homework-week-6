...
```{r}
v <- seq(from = 100, to = 500, by = 5)
m <- lm(data = d, MaxLongevity_m ~ d$Brain_Size_Species_Mean)
ci <- predict(m, newdata = data.frame(brainsize = v), interval = "confidence", level = 0.90)
pi <- predict(m, newdata = data.frame(brainsize = v), interval = "prediction", level = 0.90)
